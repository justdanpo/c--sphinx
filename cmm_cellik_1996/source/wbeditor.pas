{******************************************************}
{       Modified Turbo Vision Unit for editor          }
{       Memo crud has been striped                     }
{       3 4 5 6 key bug has been patched               }
{       Used BUGFIX ideas from French Dude             }
{******************************************************}
...
VAR transfilename : string[128];
        { filename pipe to get out of the shit of OOP }
        { it is used to find out the current editor   }
        { filename, I know no other way at this time. }
...
const                   { EDITOR flags }
  ...
  efDoReplace       = $0010;
  efBackupFiles     = $0100;
  EditorFlags: Word = efPromptOnReplace + efReplaceAll;
...
@@1:	LODSW
	MOV	BX,AX
	LODSW
{	OR	BL,BL   BUGFIX is to remove this}
{	JE	@@2     BUGFIX is to remove this}
	CMP	BL,DL
	JNE	@@3
	OR	BH,BH
	JE	@@4
{ @@2: }    	CMP	BH,DH      { removed as BUGFIX }
	JE	@@4
@@3:	LOOP	@@1
	XOR	AX,AX
@@4:	POP	DS
...
var
  ShiftState: Byte absolute $40:$17;     { added as BUGFIX }
  Key: Word;
begin
  if Event.What = evKeyDown then
  begin
    { start of BUGFIX addition }
    IF (ShiftState and $03 <> 0) AND (Event.ScanCode >= $47) AND
           (Event.ScanCode <= $51) THEN
        Event.CharCode := #0;
    { end of BUGFIX addition }
    Key := Event.KeyCode;
    if KeyState <> 0 then
    begin
    ...
    KeyState := 0;
    if (Key <> 0) { AND (key <> 20) AND (key <> 21) AND (key <> 22) AND
            (KEY <> 24) THIS was the old bugfix } then
        if Hi(Key) = $FF then
          begin
            KeyState := Lo(Key);
            ClearEvent(Event);
...
BEGIN
  ...
  IF NOT IsClipboard THEN
    BEGIN
      ...
      SetCmdState(cmCompile, True);
      SetCmdState(cmCompileAndRun, True);
...
begin
  ...
  ELSE
    BEGIN
      fsplit(Editor^.FileName,dir,name,extension);
      IF extension = '.DOC' THEN
        BEGIN
          IF name = 'WBHELP' THEN
              GetTitle := 'Work Bench HELP'
          ELSE IF name = 'STAKPROC' THEN
              GetTitle := 'Stack Procedure HELP'
          ELSE IF name = 'REGPROCS' THEN
              GetTitle := 'REG Procedure and Macro HELP'
          ELSE IF name = 'ALLPROCS' THEN
              GetTitle := 'All Procedure List HELP'
          ELSE IF name = 'C--INFO' THEN
              GetTitle := 'C-- Language HELP'
          ELSE GetTitle := Editor^.FileName;
        END
      ELSE IF extension = '.MAP' THEN
           GetTitle := 'MAP FILE for '+dir+name
      ELSE GetTitle := Editor^.FileName;
      transfilename := Editor^.FileName;
    END
END;
