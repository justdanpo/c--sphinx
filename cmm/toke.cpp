...
{
	0x53, 0x50, 0x48, 0x49, 0x4E, 0x58, 0x43, 0x2d, 0x2d, ver1, ver2
};	//надпись SPHINXC--ver
...
int CheckMacros()
{
	...
	for (m = 0; m < m_end; m++)if (strcmp(macroname[m], (char*)string) == 0)
		{
			break;
		}
			...
			else
			{
				...
				else
				{
					...
					case tk_minus:
						...
						case tk_number:
							if (i == tk_dword && (itok.rm == tk_float || itok.rm == tk_double || itok.rm == tk_qword))
							{
								i = itok.rm;
							}
...
endef:
	...
	case d_pragma:
		...
		for (i = 0; i < p_end; i++)if (strcmp(itok.name, pragmalist[i]) == 0)
			{
				break;
			}
...
void doprestuff()  //инициализация начального кода, like resize mem, jump to main...
{
	...
	if (jumptomain == CALL_NONE)
	{
		...
		if (comfile == file_com && fargc == 0 && parsecommandline == 0 && atex == 0 && use_env == 0
				&& clearpost == 0 && resizemem == 0 && killctrlc == 0)
		{
			goto endp;
		}
	}
