				...
				if (tok == tk_string)
				{
					...
					if (tok == tk_camma)
					{
							...
							if (tok == tk_camma)
							{
								...
								if (tok == tk_camma)
								{
									do
									{
										...
										{
											*(unsigned long*)&resbuf[curposbuf + 4] |= GetFlag((_STRINGS_*)&typemenu, NUMMENUPOPUP);
										}
						...
						else
						{
							if (tok != tk_camma)
							{
								*(unsigned short*)&resbuf[curposbuf + 2] = (unsigned short)GetNumber(2);
							}
							...
							if (tok == tk_camma)
							{
								do
								{
									...
									{
										*(unsigned short*)&resbuf[curposbuf] |= GetFlag((_STRINGS_*)&typemenu, NUMMENUPOPUP);
									}
	...
	while (!CloseBlock() && tok != tk_eof)
	{
		...
		*(unsigned long*)&resbuf[curposbuf] = defdialog[restok].style | 0x50000000;
		...
		case rc_control:
			...
			{
				*(unsigned short*)&resbuf[curposbuf + 16] = (unsigned short)GetNumber(2);
			}
			...
			if (tok == tk_number)
			{
				*(unsigned short*)&resbuf[curposbuf + (exts == TRUE ? 18 : 20)] = (unsigned short)itok.number;
			}
			else
			{
				if (tok == tk_string)
				...
				{
					*(unsigned short*)&resbuf[curposbuf + (exts == TRUE ? 18 : 20)] = (unsigned short)i;
				}
			}
		...
		case rc_scrollbar:
			...
			{
				*(unsigned short*)&resbuf[curposbuf + 16] = (unsigned short)GetNumber(i);
			}
