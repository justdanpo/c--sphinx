			...
			else
			{
				if (strinf.bufstr)
				{
					...
					if ((newreg = CheckIDXReg(strinf.bufstr, strinf.size, reg)) != NOINREG)
					{
						if (newreg != SKIPREG)
						{
							if (am32 == FALSE && newreg != BX && newreg != DI && newreg != SI && newreg != BP)
							{
								goto noopt;
							}
...
int  outcmp(int swapped, int ctok, ITOK* cstok, char*& cbuf, SINFO* cstr, int ctok2, ITOK* cstok2, char*& cbuf2, SINFO* cstr2, int typet)
{
	...
	case tk_reg64:
		...
		default:
			...
			{
				return (outcmp(1, ctok2, cstok2, cbuf2, cstr2, ctok, cstok, cbuf, cstr, typet));
			}
				...
				if ((tok >= tk_overflowflag) && (tok <= tk_plusflag))
				{
					...
					if (tok != tk_closebracket && (tok == tk_notequal || tok == tk_equalto))
					{
						...
						if ((tok >= tk_overflowflag) && (tok <= tk_plusflag))
						{
							...
							{
								if (oper == tk_oror && ((ittok == 5 && ittok2 == 8) || (ittok == 13 && ittok2 == 0)))
								{
									ittok = 14;
								}
								else
							...
							{
								if (oper == tk_andand && ((ittok == 6 && ittok2 == 8) || (ittok == 14 && ittok2 == 0)))
								{
									ittok = 15;
								}
								else
		...
		else
		{
			...
			else
			{
				if (tok == tk_bits)
				{
					...
					if ((ittok == tk_reg32 || ittok == tk_reg || ittok == tk_beg) && htok.number == 0)
					{
						i = CX;
					}
...
en2:
	...
	if ((ittok == tk_number || ittok == tk_postnumber) && (ittok2 == tk_number || ittok2 == tk_postnumber))
	{
		usereg = 0;
	}
	...
	if (comconst)
	{
		if (comparetok == tk_equalto || comparetok == tk_notequal)
		{
			...
			if (notflag)
			{
				comconst->typevar = (comparetok == tk_equalto ? tk_notequal : tk_equalto);
			}
		}
	}
...
#endif
{
	...
	do
	{
		...
		if ((rcompr = constructcompare(0, outptr, comconst)) == voidcompr || rcompr == zerocompr)
		{
			i = 1;
		}
		...
		if ((rcompr = constructcompare(0, outptr)) == voidcompr || rcompr == zerocompr)
		{
			i = 1;
		}
		...
		if (tok == tk_return || tok == tk_RETURN)
		{
			...
			for (unsigned int i = 0; i < *numcomp; i++)
			{
				if ((icomp + i)->type != tk_oror)
				{
					if (am32 == FALSE)
					{
						*(unsigned short*)&output[(icomp + i)->loc - di] = (unsigned short)(outptr - (icomp + i)->loc);
					}
					else
					{
						*(unsigned long*)&output[(icomp + i)->loc - di] = (unsigned long)(outptr - (icomp + i)->loc);
					}
				}
			}
...
merge_if:
			...
			if (chip < 3)
			{
				for (i = 0; i < *numcomp; i++)
				{
					if ((icomp + i)->type == tk_oror)
					{
						output[(icomp + i)->loc - 1] = (unsigned char)(output[(icomp + i)->loc - 1] - 3 - j);
					}
					else
					{
						if ((icomp + i)->type != tk_andand)
						{
							output[(icomp + i)->loc - 5 - j] = (unsigned char)(output[(icomp + i)->loc - 5 - j] ^ 1);
						}
						else
						{
							if (am32 == FALSE)
							{
								*(unsigned short*)&output[(icomp + i)->loc - 2] = (unsigned short)(outptr - (icomp + i)->loc);
							}
							else
							{
								*(unsigned long*)&output[(icomp + i)->loc - 4] = (unsigned long)(outptr - (icomp + i)->loc);
							}
						}
					}
				}
			...
			else
			{
				for (i = 0; i < *numcomp; i++)
				{
					if ((icomp + i)->type == tk_oror)
					{
						output[(icomp + i)->loc - 1] = (unsigned char)(output[(icomp + i)->loc - 1] - 2 - j);
					}
					else
					{
						if ((icomp + i)->type != tk_andand)
						...
						else
						{
							if (am32 == FALSE)
							{
								*(unsigned short*)&output[(icomp + i)->loc - 2] = (unsigned short)(outptr - (icomp + i)->loc + 1);
							}
							else
							{
								*(unsigned long*)&output[(icomp + i)->loc - 4] = (unsigned long)(outptr - (icomp + i)->loc + 1);
							}
						}
					}
				}
...
nooptim:
		...
		for (unsigned int i = 0; i < numcomp; i++)
		{
			if ((icomp + i)->type != tk_oror)
			{
				if (am32 == FALSE)
				{
					*(unsigned short*)&output[(icomp + i)->loc - 2] = (unsigned short)(outptr + j - (icomp + i)->loc);
				}
				else
				{
					*(unsigned long*)&output[(icomp + i)->loc - 4] = (unsigned long)(outptr + j - (icomp + i)->loc);
				}
			}
		}
...
#endif
{
	...
	do
	{
		...
		if ((rcompr = constructcompare(1, outptr, comconst)) == voidcompr || rcompr == zerocompr)
		{
			i = 1;
		}
		...
		if ((rcompr = constructcompare(1, outptr)) == voidcompr || rcompr == zerocompr)
		{
			i = 1;
		}
...
void dowhilefast(unsigned int typeb)
{
	...
	do
	{
		...
		if ((rcompr = constructcompare(2, startloc, NULL)) == voidcompr || rcompr == zerocompr)
		{
			i = 1;
		}
		...
		if ((rcompr = constructcompare(2, startloc)) == voidcompr || rcompr == zerocompr)
		{
			i = 1;
		}
...
void setproc(int defflag)
{
	...
	if (searchteg && (!(current_proc_type & f_static)) && (current_proc_type & f_typeproc) != tp_pascal)
	{
		AddThis();
	}
	...
	if (inlineflag == 0 && retproc == FALSE && (i || (lastcommand != tk_goto && lastcommand != tk_GOTO)))
	{
		leaveproc();
	}
...
void insertproc(/*int sizepar*/)
{
	...
	if (inlineflag == 0 && (localsize || paramsize) && (ESPloc == FALSE || am32 == FALSE))
	{
		Leave();
	}
...
void CorrectParamVar()
{
	...
	if ((current_proc_type & f_typeproc) == tp_cdecl || (current_proc_type & f_typeproc) == tp_stdcall)
	{
		fspin = FALSE;
	}
	else
	...
	for (ptr = ntlr->lrec;; ptr = ptr->rec.next)
	{
		if (ptr->rec.type == tp_paramvar)
		{
			if (fspin)
			{
				ptr->rec.recnumber = paramsize - ptr->rec.recnumber - Align(ptr->rec.recsize, am32 == FALSE ? 2 : 4);
			}
	...
	do
	{
		...
		if (flag)
		{
			do
			{
				...
				if ((sparamtok == tk_qwordvar || sparamtok == tk_doublevar) && numpointr == 0)
				{
					lsize = 8;
				}
					...
					(FindTeg(FALSE) == NULL && FindTeg(TRUE) == NULL))
			{
				...
				else
				{
						...
						if (!finline)
						{
							...
							if (SizeBackBuf)
							{
								...
								if (size)
								{
									...
									if (binptr)
									{
										if (!optimizespeed && type < 3 && (type * (am32 + 1) * 2) >= binptr)for (; type != 0; type--)
											{
												op(0x50 + ECX);
											}
										else
					...
					if (psavereg->size)
					{
						...
						if (ESPloc && am32 && (itok.type == tp_localvar || itok.type == tp_paramvar))
						{
							itok.number += addESP;
						}
					}
								...
								|| tok == tk_undefofs || tok == tk_postnumber))
						{
							...
							if (OnlyNumber(i))
							{
								...
								else
								{
									...
									{
										(oitok.flag & f_extern) == 0 ? setwordpost(&oitok) : setwordext(&oitok.number);
									}
					...
					else if (vartype < tk_long)
					{
						...
						case tk_seg:
							...
							else
							{
								...
								if ((unsigned int)itok.number <= GS)if (cpu < 3)
									{
										cpu = 3;
									}
							}
				...
				(postbuf + count)->num == which && (postbuf + count)->loc >= top)
		{
			...
			else 	//SHORT
			{
				...
				else
				{
					...
					{
						preerror3("BREAK distance too large, use break", (postbuf + count)->line);
					}
					else if ((postbuf + count)->type == CONTINUE_SHORT)
					{
						preerror3("CONTINUE distance too large, use continue", (postbuf + count)->line);
					}
					else
					{
						preerror3(shorterr, (postbuf + count)->line, (postbuf + count)->file);
					}
				}
			}
			...
			if (hold < 127)
			{
				...
				if ((postbuf + count)->type == BREAK_NEAR || (postbuf + count)->type == BREAK_32)
				{
					warningjmp("BREAK", (postbuf + count)->line);
				}

				if ((postbuf + count)->type == CONTINUE_NEAR || (postbuf + count)->type == CONTINUE_32)
				{
...
void  outseg(ITOK* outtok, unsigned int locadd)
{
	...
	if (outtok->post != 0 && outtok->post != UNDEF_OFSET)
	{
		...
		{
			(postbuf + posts)->type = (unsigned short)(am32 == 0 ? POST_VAR : POST_VAR32);
		}
...
void dopoststrings()
{
	...
	for (i = 0; i < posts; i++)
	{
		...
		if ((postbuf + i)->type == POST_STRING)
		{
			if (segm == CS)
			{
				*(unsigned short*)&output[(postbuf + i)->loc] += (unsigned short)addvalue;
			}
			else
			{
				*(unsigned short*)&outputdata[(postbuf + i)->loc] += (unsigned short)addvalue;
			}
		...
		else if ((postbuf + i)->type == POST_STRING32)
		{
			...
			{
				(postbuf + i)->type = (unsigned short)(segm == DS ? FIX_VAR32 : FIX_CODE32);
			}
		}
	}
...
idrec* addtotree(char* keystring)//добавить строку в дерево
{
	...
	if (ptr == NULL)
	{
		((itok.flag & f_static) != 0 ? staticlist : treestart) = newptr;    //начало дерева
	}
...
jump labels. */
{
	...
	for (ftlr = btlr; ftlr != NULL;)
	{
		...
		for (ptr = ftlr->lrec; ptr != NULL;)
		{
			...
			{
				if (ptr->rec.type == tp_localvar || ptr->rec.type == tp_postvar || ptr->rec.type == tp_gvar)
				{
...
void AddRetList(int pos, int line, int type)
{
	...
	{
		listreturn = (RETLIST*)REALLOC(listreturn, sizeof(RETLIST) * (numreturn + 1));
	}
...
int testInitVar(int checkaldef)
{
	...
	for (;;)
	{
		...
		else if (tok == tk_id)
		{
			...
			if (tproc == 0)
			{
				tproc = (comfile == file_w32 ? tp_stdcall : tp_pascal);    //тип проц по умолчанию
			}
			else
			{
...
classdecl:
		...
		else if (tok == tk_undefproc || tok == tk_declare/*||tok==tk_apiproc*/)
		{
					...
					((flag & f_static) && (itok.flag & f_static) == 0))
			{
				...
				{
					if (rettype == itok.rm || (rettype == tokens && (itok.rm == (am32 == 0 ? tk_word : tk_dword))))
					{
						break;
					}
...
void unpackteg(structteg* tteg)
{
	...
	for (i = 0; i < tteg->numoper; i++)
	{
		...
		case tk_structvar:
			...
			if (ptr == NULL)
			{
				((tteg->flag & f_static) == 0 ? treestart : staticlist) = newrec;    //начало дерева
			}
			else 	//поиск строки в дереве
			{
...
void RegAddNum(int reg)
{
	if ((!structadr.post) && optnumadd(structadr.number, reg, am32 == FALSE ? r16 : r32, 0) != FALSE)
	{
		return;
	}
