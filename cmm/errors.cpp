...
void warningoptnum()
{
	if (wact[0].usewarn)
	{
		wact[0].fwarn("Optimize numerical expressions", linenumber, currentfileinfo);
	}
}
...
void warninline()
{
	if (wact[11].usewarn)
	{
		wact[11].fwarn("Don't use local/parametric values in inline functions", linenumber, currentfileinfo);
	}
}
...
void warnsize()
{
	if (wact[12].usewarn)
	{
		wact[12].fwarn("Sources size exceed destination size", linenumber, currentfileinfo);
	}
}
...
void warpragmapackpop()
{
	if (wact[13].usewarn)
	{
		wact[13].fwarn("Pragma pack pop with no matching pack push", linenumber, currentfileinfo);
	}
}
	...
	for (ftlr = btlr; ftlr != NULL; ftlr = ftlr->next)
	{
		for (ptr = ftlr->lrec; ptr != NULL; ptr = ptr->rec.next)
		{
			...
			if (i == tp_postvar || i == tp_gvar || i == tp_localvar || i == tp_paramvar)
			{
				...
				if (ptr->rec.npointr)for (i = ptr->rec.npointr; i > 0; i--)
					{
						fputc('*', hmap);
					}
