					...
					unsigned int* rflag, ITOK* posttok)
{
	...
	for (;;)
	{
		...
		if (tok == tk_mult && (tok2 != tk_number || *zoom != -1 || *val != 0 || *idx != -1))
		{
			break;
		}
...
con1:
		...
		if (tok == tk_minus)
		{
			if (tok2 == tk_postnumber || (tok2 == tk_number && (itok2.flag & f_reloc)))
			{
				break;
			}
...
int Reg32ToLea2(int reg)	//оптимизация сложения 32-битных регистров в LEA
{
	...
	if (tok == tk_minus && (!((tok2 == tk_number && itok2.rm != tk_float && itok2.rm != tk_double) || tok2 == tk_postnumber)))
	{
		return FALSE;
	}

	if (tok == tk_mult && (!(tok2 == tk_number && itok2.rm != tk_float && itok2.rm != tk_double && (itok2.flag & f_reloc) == 0)))
	{
		return FALSE;
	}

	if (tok == tk_plus && tok2 != tk_reg32 && tok2 != tk_number && tok2 != tk_postnumber && (!(tok2 == tk_rmnumber && (itok2.flag & f_useidx) == 0)))
	{
		return FALSE;
	}
...
int Reg32ToLea(int reg)	//оптимизация сложения 32-битных регистров в LEA
{
	...
	if (tok != tk_reg32 && tok != tk_number && tok != tk_postnumber && (!(tok == tk_rmnumber && (itok.flag & f_useidx) == 0)))
	{
		return FALSE;
	}
	...
	if (tok == tk_mult)
	{
		...
		{
			if ((itok.flag & f_reloc) && (itok.rm == tk_float || itok.rm == tk_double || itok.rm == tk_qword))
			{
				goto retfalse;
			}
...
void CheckRegForLea16(int reg, int* idx, int* base, unsigned long* val, unsigned int* rflag, ITOK* posttok)
{
	...
	for (;;)
	{
		if (tok != tk_plus && (!(tok == tk_minus && ((tok2 == tk_number && itok2.rm != tk_float && itok2.rm != tk_qword && itok2.rm != tk_double) || tok2 == tk_postnumber))))
		{
			break;
		}
...
int Reg16ToLea(int reg)	//оптимизация сложения 16-битных регистров в LEA
{
	...
	if (!((tok == tk_reg || (tok == tk_number && itok.rm != tk_float && itok.rm != tk_double) && tok == tk_postnumber) && tok2 == tk_plus))
	{
		return FALSE;
	}
...
void RegMulNum(int reg, unsigned long num, int razr, int sign, int* expand, int flag)
{
	...
	default:
		...
		if (chip < 7 && *expand == FALSE && optimizespeed && speedmul32(num, reg, razr))
		{
			break;
		}
...
int MultiAssign(int razr, int usereg, int npointr)
{
	...
	case tk_reg32:
		if (wtok.number != hnumber)
		{
			...
			{
				waralreadinitreg(regs[nrazr / 4][wtok.number], regs[nrazr / 4][hnumber]);
			}
		}
...
regtovar:
			...
			case tk_seg:
				...
				if ((unsigned int)itok.number == FS || (unsigned int)itok.number == GS)if (cpu < 3)
					{
						cpu = 3;
					}
			...
			case tk_longvar:
			case tk_dwordvar:
				if ((rettype == tk_long || rettype == tk_dword) && hnumber && regoverstack)
				{
					goto pushvar;
				}
...
optpent:
		...
		if ((chip >= 3 && (!optimizespeed)) || RmEqualReg(AX, gstok->rm, gstok->sib))
		{
			goto movxx;
		}
...
swapreg:
	...
	case tk_plusequals:
		...
		if (itok2.type == tp_opperand && tok != tk_number && tok != tk_undefofs && tok != tk_postnumber)
		{
			goto defadd;
		}
		...
		case tk_number:
			...
			if (itok.type == tp_opperand)
			{
				...
				if (i == tk_postnumber || i == tk_undefofs)
				{
					...
					if (i == tk_postnumber)
					{
						(postnumflag & f_extern) == 0 ? setwordpost(&itok) : setwordext((long*)&ii);
					}
					else
					{
			...
			if ((postnumflag & f_reloc) == 0 && i != tk_undefofs && i != tk_postnumber && optnumadd(ii, reg, razr, vop))
			{
				break;
			}
...
// all other registers preserved
{
	...
	while (itok.type != tp_stopper && tok != tk_eof)
	{
		...
		case tk_plus:
			...
			{
			case tk_number:
				if ((itok.flag & f_reloc) == 0 && optnumadd(itok.number, reg, razr, vop))
				{
					break;
				}
...
void fwait3()
{
	if (chip < 4 && tok2 != tk_floatvar && tok2 != tk_doublevar && tok2 != tk_float && tok2 != tk_double)
	{
		op(0x9B);
	}
}
...
void setwordpost(ITOK* stok)						/* for post word num setting */
{
	...
	{
		(postbuf + posts)->type = (unsigned short)(am32 == FALSE ? POST_VAR : POST_VAR32);
	}
