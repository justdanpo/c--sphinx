			...
			if (tok == tk_case || tok == tk_CASE || tok == tk_default) //если case - запомнить позицию и величину
			{
				...
				if (mode == 0)
				{
					if (numcase == 0 && defaul) 	//default самый первый
					{
						if (am32 == FALSE)
						{
							*(unsigned short*)&output[defaul - 2] = (unsigned short)(outptr - defaul);
						}
						else
						{
							*(unsigned long*)&output[defaul - 4] = (unsigned long)(outptr - defaul);
						}
					}
					else if (numcase)
					{
						CheckJmpSW(linenumber, (caseinf + numcase)->postcase, outptr, tok == tk_case ? FALSE : TRUE, mesCASE);
					}
				}
				else
				{
