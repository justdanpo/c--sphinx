...
void CompareRegStat(REGISTERSTAT* bak)
{
	...
	for (int i = 0; i < 8; i++)
	{
		if ((regstat + i)->type == t_undef || (regstat + i)->type != (bak + i)->type ||
				((regstat + i)->type == t_id && strcmp((regstat + i)->id, (bak + i)->id) != 0))
		{
			ClearRegBak(i, bak);
		}
	}
...
int Const2Var(ITOK* itok4, long long num, int typenum)
{
	...
	if (replasevar && itok4->rec && (itok4->flag & f_useidx) == 0)
	{
		...
		else if ((listvic + i)->contype == typenum && (listvic + i)->lnumber == num)
		{
			return FALSE;
		}
...
{
	if (replasevar == FALSE || itok4->rec == NULL || (itok4->flag & f_useidx) != 0)
	{
		return FALSE;
	}
